...
| Bank maintenance | Load the supplied RAM tool to map banks, copy and verify 32K banks, adopt existing payloads, reclaim stale D0-D2 rows after an erased-bank proof, erase guarded ranges, and install the narrow AP carrier | Reclaim requires exact confirmation and rewrites/verifies the complete protected Bank-3 sector F while preserving all other bytes |
