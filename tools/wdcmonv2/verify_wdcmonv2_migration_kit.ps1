...
if ($manifest.schema -ne 1 -or
    ...
    $manifest.archiveRoot -ne 'STR8-N-v1.33-Migration-Kit') {
...
foreach ($required in @('UNTESTED ON LINUX HARDWARE', 'Windows 11 PowerShell remains the board-proven reference',
        'COPY B3 TO B0', 'INSTALL STR8-N 1.33', 'READ THE SCREEN', 'CTRL+U once',
...
foreach ($required in @('COPY B3 TO B0', 'INSTALL STR8-N 1.33', 'PROPOSED D0 B3:$FFB0',
...
foreach ($required in @('Windows PowerShell 5.1', 'USB COM-port driver',
        ...
        'ARCHIVE ROOT: STR8-N-v1.33-Migration-Kit', 'READ THE SCREEN',
...
Write-Host 'HARDWARE STATUS      = V1.33 HOST-QUALIFIED; V1.32 FACTORY PATH BOARD-ACCEPTED'
