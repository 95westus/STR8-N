# STR8-iN/65 v1.31 RAM Bank Maintenance

This guide applies to the production STR8-iN/65 v1.31 RAM image:

```text
BUILD/v1.31/s19/str8n-v1.31-str8-in65-bank-maint-2000.s19
```

It is part of the v1.31 factory migration ZIP under the consumer-facing name
`ARTIFACTS/STR8-iN65-BANK-MAINT-2000.s19`. The resident cold-start and EDU
quiet-start sequence it supports is canonical v1.31; the tool's WDC-specific
...
and installs the exact canonical STR8-N v1.31 top with an erased Bank-3
directory. This RAM maintenance image is therefore a required consumer step:
it explicitly publishes D0 only after the first verified v1.31 boot. `J0`
...
defaults. The v1.31 consumer procedure keeps type `FF` and deliberately
...
- B3:`$FFF0` WORK and `$FFF1` top-backup role bytes are preserved exactly by
  D0 adoption; the canonical v1.31 image initializes them to B1:E and B1:F.
