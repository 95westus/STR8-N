...
Version 1.33 is the maintained version. It retains the v1.32 reset-source and
immediate-banner behavior and passed guarded top update, exact readback,
...
passed on COM4 on 2026-09-08; v1.33 migration artifacts are host-qualified but
...
STR8-N 1.33 only in B3:F, and then guides the operator through publishing the
...
STR8-N 1.33 image reports `BAD`, returns to `STR8-N>`, and does not execute the
...
### Load the STR8-N 1.33 bank-maintenance program

Use `BUILD/v1.33/s19/str8n-v1.33-bank-maint-2000.s19`. It is a temporary RAM tool;
...
2. When `S19` appears, send
   `BUILD/v1.33/s19/str8n-v1.33-bank-maint-2000.s19` at normal full speed.
...
### Upgrade Bank 3 sector F to STR8-N v1.33

The v1.33 release includes the RAM-resident updater
`BUILD/v1.33/s19/str8n-v1.33-top-update-2000.s19`. It is loaded by the existing
...
1. Keep both the v1.1 rollback BIN and
   `BUILD/v1.33/bin/str8n-v1.33-bank3-f000-ffff.bin` off-board.
...
1. Type `L` and send `str8n-v1.33-top-update-2000.s19` at normal full speed.
...
5. Type the exact final confirmation `STR8-N 1.33`.
...
to retry the embedded v1.33 image or `O` to restore the verified Bank-1 backup.
...
After v1.33 starts, verify `S`, `C`, `W`, selector timeout, the
...
STR8-N 1.33 `SR 02 03` service signature. `Q` starts the normal STR8-N startup
display again; the timing interval is deliberately silent in v1.33.
...
`BUILD/v1.33/s19/str8n-v1.33-directory-refresh-2000.s19` is a dedicated
...
2. Require the title `STR8-N 1.33 DIRECTORY REFRESH` and
...
powershell -NoProfile -ExecutionPolicy Bypass -File tools/build_directory_refresh_image.ps1 `
  ...
  -OutPath "BUILD/v1.33/bin/board-directory-refreshed-20000.bin"
```
