...
The companion R-YORS repository retains the role migration sequence at
`DOC/GUIDES/AP/SECTOR_ROLES_AND_RAM_TRANSIENTS_2026-09-16.md`.

Version 1.35 is current; see its [update/reset proof](STR8N_V1_35_BOARD_TEST_2026-09-16.md).
Factory migration and the broader hardware matrix remain unqualified for 1.35.
The preceding version 1.34 passed the historical
...
in v1.34. They poll `0`, `1`, `2`, `C`, `W`, or `S`. `C` cold-starts compatible
...
```text
$F000-$FCF1  resident supervisor, installer, loader   3314 bytes
...
The bytes between `_END_DATA` and the packed worker are available resident
growth room. The reset-source addition deliberately uses the previously
reserved growth budget; the layout checker now requires at least 8 bytes of
margin and rejects any overlap. See the
[reset-source contract](RESET_SOURCE_CONTRACT.md), the
[LED status implementation record](LED_STATUS_PROPOSAL.md), the
[minimal-slice board proof](LED_STATUS_BOARD_TEST_2026-09-06.md), the
[host-presence follow-on proof](LED_HOST_PRESENCE_BOARD_TEST_2026-09-06.md),
the [I/O activity proof](LED_IO_ACTIVITY_BOARD_TEST_2026-09-06.md),
and the preceding [conservative-pass evidence](STR8N_CONSERVATIVE_RESIDENT_PASS.md).
`$FF` bytes found inside linked code are not automatically free space.

The stored worker is copied to `$0200-$0437` before an install or bank handoff.
...
`BUILD/v1.35/s19/str8n-v1.35-worker-0200.s19` is build and integration evidence, not a file
to send to `I`.
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -BaseAddress 32768 `
  -Bank 0 `
  -S19Path BUILD/v1.35/s19/guest-bank0-8000-ffff.s19
```
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -EntryAddress 49152 `
  -Bank 3 `
  -S19Path BUILD/v1.35/s19/himon-bank3-c000-efff.s19
```
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -PayloadS19Path BUILD/v1.35/s19/guest-bank0-8000-ffff.s19 `
  -PayloadStart 32768 `
  -PayloadEndExclusive 65536 `
  -Bank 0 `
  -S19Path BUILD/v1.35/s19/str8n-i-guest.s19
```
...
### STR8-N 1.35 bank-maintenance RAM image

`make bank-maint` builds and validates
`BUILD/v1.35/s19/str8n-v1.35-bank-maint-2000.s19`. Its S9 entry is `$2000`; the S1
address span is `$2000-$39B2`, wholly inside the `L` contract. The fixed
private worker remains at `$3400-$362A`; the guarded rename extension follows
it without changing the worker bytes. The WDC linker
fills the unused space before the embedded worker, so the stream contains
6,579 RAM data bytes even though the executable regions are smaller.
...
separate `R` path can rewrite it only after either an erased-bank proof and
exact directory-clear confirmation or an exhausted-D3-journal proof, automatic
erased-scratch selection, and exact `RESET J3` confirmation. D3 compaction
...
See [configuration bytes and prerequisites](CONFIGURATION_BYTES.md) for the
complete `$FFF0-$FFF9` map, all FNV-policy encodings, manager dependencies,
role migration, directory fields, vectors and guarded-update behavior.
...
`str8n-v1.35-directory-refresh-2000.s19` contain an all-`$FF` directory and
...
three allowed-bank bits. R-YORS's matching HIMON/AM03 resolver consumes this constant;
...
$00A1-$00A2  I range state                         2 bytes
$00A3        free; unread sector count removed     1 byte
$00CD-$00D6  record/directory/worker scratch      10 bytes
$0200-$0437  relocated unified worker            568 bytes
$0A00-$19FF  current 4K sector tray             4096 bytes
$1A00-$1FFF  STR8-N-free low RAM                 1536 bytes
$7C00-$7DBF  foreground High Tool Overlay         448 bytes
$7DC0-$7DC7  HIMON AP-link scratch                  8 bytes
$7DC8-$7DE6  reserved                              31 bytes
$7DE7-$7DE8  one-shot software-reset record         2 bytes
...
$7EDE-$7EDF  no longer used by STR8-N delay         2 bytes
$7EED-$7EEF  IVI signature                         3 bytes
$7EF8-$7EFF  IVI RAM vectors                       8 bytes
$0100-$01FF  hardware stack, dynamic              256 bytes maximum
```

The main `I` transaction regions through the record card total 4,974 named
bytes when the `L`-only flag at `$00A0` is excluded. The fixed IVI cells add
11 bytes. The delay uses no fixed RAM. The stack can consume 256 more bytes.

```text
Quantity                                      Bytes
board RAM below I/O                          32,512
named I transaction areas                     4,974
fixed IVI cells                                  11
outside those named/fixed STR8-N areas        27,527
outside them with full stack reserved         27,271
...
helper occupies `$0300-$0335`, above the selector prefix at `$0200-$0226`.
It shares the full worker tray by phase and cannot coexist with the complete
mutation worker.

### `$1A00-$1FFF`: STR8-N-free low RAM

STR8-N v1.34 itself makes no fixed runtime allocation in `$1A00-$1FFF`.
Integrated R-YORS APMAN does transiently use `$1A00-$1AFF` as its command
shadow while `AP`, `APS`, or banked `INSTALL` delegates. Therefore
`$1B00-$1FFF` is the unconditional R-YORS user-low slice outside another phase
owner. As with all RAM, a HIMON cold start clears it, and an application must
avoid colliding with another user program that it loaded itself.
...
### STR8-N v1.34 high-RAM ABI

```text
$7C00-$7DBF  foreground High Tool Overlay
$7DC0-$7DC7  HIMON AP-link scratch
$7DC8-$7DE6  reserved
$7DE7-$7DE8  one-shot software-reset record
$7DE9-$7DFF  remaining STR8-N Recovery State Capsule
```
...
The 25-byte Recovery State Capsule contains these defined slots:

```text
$7DE7-$7DE8 software-reset signature "RS"; SIG1 is the commit byte
...
uses the public `$F009` `SR/02` parser and applies its own destination,
copy/accounting, error-latch, and load-only session policy:
...
The generated `BUILD/v1.35/include/str8n-public.inc` exports this complete
request/result contract, not only the `$F009-$F00F` discovery bytes. External
clients such as HIMON must consume that generated contract and verify `SR/02`
plus the required source capability before issuing a request.
...
verifies the 39-byte selector prefix at `$0200-$0226`, then tail-calls its
...
clobbered. The copied selector overwrites `$0200-$0226`.
...
make led-worker-test         build the guarded LED/production-worker probe
make size-optimization-check execute resident/worker differential tests (py65)
...
make layout-check            enforce fixed addresses and no worker overlap
...
make str8-in65-bank-maint    build the isolated migration maintenance S19
make str8-in65-promotion-check
...
placement, exact image size, and at least 128 bytes between resident and worker.
...
These host fixtures are written below `BUILD/v1.35/test/range-matrix`; they do not
...
BUILD/v1.35/s19/str8n-v1.35-f000.s19         resident build component
BUILD/v1.35/s19/str8n-v1.35-worker-0200.s19  worker evidence/build component
...
BUILD/v1.35/s19/str8n-v1.35-led-worker-test-2000.s19
                                      guarded LED/production-worker probe
...
BUILD/v1.35/s19/str8n-v1.35-str8-in65-bank-maint-2000.s19
                                      isolated WDC migration maintenance tool
BUILD/v1.35/s19/str8n-v1.35-wdcmonv2-archive-2000.s19
                                      optional read-only stock-bank archive
BUILD/v1.35/s19/str8n-v1.35-wdcmonv2-install-2000.s19
                                      factory WDCMONv2 migration program
BUILD/v1.35/bin/str8n-v1.35-wdcmonv2-bank3-f000-ffff.bin
                                      exact canonical top: directory erased; roles 1E/1F
BUILD/v1.35/str8n-v1.35-wdcmonv2-str8n-migration-kit.zip
                                      allowlisted consumer migration kit
BUILD/v1.35/s19/ryors-v1.2-str8n-himon-asm-bank0-2-8-f.s19
...
root `BUILD/v1.35/`. The compatibility manifest remains at
...
Current v1.35 coverage is limited to the linked 2026-09-16 update/reset report.
Historical v1.34 evidence is the three 2026-09-15 reports linked at the start of
...
intentional physical RESET retained STR8-N `1.21` and HIMON `1303`. The fixed
HIMON head and ASM-F2 identity matched, the renamed Bank Maintenance image
mapped the installed banks without mutation, and explicit `J3` generated the
synthetic RESET-vector return to the same identities. A separate failed
...
The historical 3413-byte v1.22 host build changed the selector timeout to warm
HIMON entry, added explicit `C` cold and `W` warm entry, and applied
...
The historical v1.29 resident is 3,414 bytes at `$F000-$FD55`, leaving 6 bytes
before the fixed `$FD5C` worker. Its board-proven cold-start path keeps the
quarantine/live-selector timing but removes the former `WAIT...`/dot output
and performs no reset-time `$7FEC` bank-latch write. The 2026-08-28 factory
migration additionally proves byte-exact stock B3 preservation in B0,
COMPLETE D0 `WDCM2`, retained-monitor `J0`, the operator-observed CS0-CS3
chase, and physical RESET back to STR8-N 1.29.
...
Earlier v1.2 board sessions additionally established:

- physical RESET selects Bank 3 and reaches STR8-N;
- the recorded historical `WAIT...` pulses and later identity/selector are visible;
