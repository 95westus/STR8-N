...
After the calibrated pre-I/O delay and console initialization, STR8-N prints
`RESET`, then runs six silent quarantine ticks approximately one second apart.
...
silent in v1.31. Only this second phase polls `0`, `1`, `2`, `C`, `W`, or `S`. `C` cold-starts compatible
...
```text
$F000-$FD27  resident supervisor, installer, loader   3368 bytes
$FD28-$FD4F  currently unused margin                    40 bytes
...
growth room. The layout checker requires at least 32 bytes of margin and
rejects any overlap. The RX/TX activity slice leaves 40 bytes. See the
...
The stored worker is copied to `$0200-$045F` before an install or bank handoff.
...
`BUILD/v1.31/s19/str8n-v1.31-worker-0200.s19` is build and integration evidence, not a file
to send to `I`.
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -BaseAddress 32768 `
  -Bank 0 `
  -S19Path BUILD/v1.31/s19/guest-bank0-8000-ffff.s19
```
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -EntryAddress 49152 `
  -Bank 3 `
  -S19Path BUILD/v1.31/s19/himon-bank3-c000-efff.s19
```
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -PayloadS19Path BUILD/v1.31/s19/guest-bank0-8000-ffff.s19 `
  -PayloadStart 32768 `
  -PayloadEndExclusive 65536 `
  -Bank 0 `
  -S19Path BUILD/v1.31/s19/str8n-i-guest.s19
```
...
### STR8-N 1.31 bank-maintenance RAM image

`make bank-maint` builds and validates
`BUILD/v1.31/s19/str8n-v1.31-bank-maint-2000.s19`. Its S9 entry is `$2000`; the S1
...
`str8n-v1.31-directory-refresh-2000.s19` contain an all-`$FF` directory and
...
$0200-$045F  relocated unified worker            608 bytes
...
The main `I` transaction regions through the record card total 5,014 named
...
named I transaction areas                     5,014
fixed IVI cells                                  11
outside those named/fixed STR8-N areas        27,487
outside them with full stack reserved         27,231
...
helper begins at `$0500`, above the worker's fixed `$045F` last byte.

### `$1A00-$1FFF`: STR8-N-free low RAM

STR8-N v1.31 itself makes no fixed runtime allocation in `$1A00-$1FFF`.
...
The generated `BUILD/v1.31/include/str8n-public.inc` exports this complete
...
placement, exact image size, and at least 32 bytes between resident and worker.
...
These host fixtures are written below `BUILD/v1.31/test/range-matrix`; they do not
...
BUILD/v1.31/s19/str8n-v1.31-f000.s19         resident build component
BUILD/v1.31/s19/str8n-v1.31-worker-0200.s19  worker evidence/build component
...
BUILD/v1.31/s19/str8n-v1.31-led-worker-test-2000.s19
...
BUILD/v1.31/s19/str8n-v1.31-str8-in65-bank-maint-2000.s19
...
BUILD/v1.31/bin/str8n-v1.31-wdcmonv2-bank3-f000-ffff.bin
                                      migration top: D0 WDCM2; roles FF/FF
BUILD/v1.31/str8n-v1.31-wdcmonv2-str8n-migration-kit.zip
                                      allowlisted consumer migration kit
BUILD/v1.31/s19/ryors-v1.2-str8n-himon-asm-bank0-2-8-f.s19
...
root `BUILD/v1.31/`. The compatibility manifest remains at
